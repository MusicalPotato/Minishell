...
/*   Updated: 2021/02/25 18:39:50 by igor             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_msg_check(char *line, int count, int *quote, t_cmd *cmd)
{
	...
	{
		if ((count = ft_msg_check_2(line, count, cmd, 0)) < 0)
			return (count);
	}
	else if ((line[count] == '"' && *quote != 1) || (line[count] == '\''
				&& *quote != 2))
	{
		if ((count = ft_msg_check_1(line, count, 1, quote)) < 0)
			return (count);
	}
	else if ((line[count] == '>' || line[count] == '<') && !*quote)
	{
		if ((count = ft_msg_check_3(line, count, cmd)) < 0)
			return (count);
	}
	else if ((line[count] == '|') && !*quote)
	{
		if ((count = ft_msg_check_2(line, count, cmd, 1)) < 0)
			return (count);
	}
	else
		if (!(cmd->arg[cmd->arg_nbr - 1] = ft_memcat(cmd->arg[cmd->arg_nbr - 1],
